...
大部分"AI 游戏"工具止步于 prompt 出原型。Vitric 围绕一套 **agent API** 来造，让 AI 能自主把一个游戏一路做到可发行：**跑游戏 → 看像素和状态 → 断言 → 改 → 再来一遍**，全程不需要人。现有引擎是为坐在编辑器前的人设计的，对 AI 是黑盒——状态看不见、bug 复现靠猜、"做完"只是 AI 的一句话不是事实。Vitric 反过来：引擎里每一份状态都看得见、操作得了、验证得了，又因为模拟是逐位确定的，引擎能**证明**关于一个游戏的事——比如某段录像确实能通关、一群 agent 怎么玩都卡不死它。`games/` 下已有两款完整游戏通过交付门禁。
