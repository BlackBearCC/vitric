...
vitric install <module> [--project <dir>]  # install a gameplay module from the built-in registry into a project (copies files + updates vitric.json includes); `vitric install --list` shows available modules
...
- **Activation by name**: `input/ui-click-by-name {name, button?}` — activate a button by its scene entity name (e.g. `"mode_craft"`, `"craft_plank"`) instead of coordinates, running the same `activate_button` path as coordinate clicks (sets pressed + press_t + moves focus + emits `ui-activate`). **Layout-independent**: changing `mode_row`'s gap or button widths won't break scripts. **Stricter** than coordinate clicks — missing name / entity has no `Button` component / button is `Disabled` all error out (coordinate clicks silently miss off-rect). Recordings store `{name, button}`; names are deterministic scene state, replays are bit-for-bit. Agent scripts should prefer by-name; reserve coordinate clicks for "I don't know the button name, just roughly where it is" exploration.
