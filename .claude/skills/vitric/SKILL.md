...
rpc '{"method":"sim/quit"}'
# 给人看整局回放(web端): 每隔若干tick抓 sim/snapshot 收成流+关键帧标动作 →
#   python scripts/build_replay.py <流.json> <out.html> 出自包含HTML回放页(操作日志+玩家高亮+图例,纯数据无视频)
#   抓取范例 games/frontier/tools/capture_replay.py;详见 docs/agent-guide.md「给人看整局回放」
```
