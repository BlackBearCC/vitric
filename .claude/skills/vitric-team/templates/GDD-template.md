...
## 美术方向（导演与需求方定，美术照此执行，不许自带默认风格）
- 风格词：<如 厚涂墨线暗黑哥特 / 16-bit 像素 / 水彩手绘 / 扁平极简——写完整的提示词锚点>
- 分辨率档：<像素=小图+`vitric assets --height H`；高清=素材像素≥最大显示像素，严禁放大>
- 字体：<点阵(ASCII,配像素) / TTF(manifest font 字段,比例排版+中文,配高清)>
- 法线策略：<程序化 --normals / AI --normals-ai / 平面占位(无缝平铺图)>
