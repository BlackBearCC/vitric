...
examples/coin-run  示例游戏（规则+脚本+断言全覆盖的端到端测试就在这）
```
...
核心闭环已跑通（78 个测试）：确定性录像回放、语义观察（render/describe）、热重载、精灵贴图+素材校验、窗口呈现+检查器（点选/拖拽写回，选中态人机双向可见）、MCP server。进行中：wgpu GPU 渲染、音频、动画状态机、关卡配方生成、运行时 LLM 模块。
