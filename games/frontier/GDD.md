...
**地图/探索(v1 单场景双区,稳)**:一张大地图 `main`(约 28×14),**左半=家园区**(建造/种田/住人)、**右半=野外区**(采集/撞旅人),中间一道气闸/走廊连通。玩家走过去即"切到"野外,相机 follow、一次只见半屏 → 有探索感。不用 `load-scene`/`Persist`(引擎跨场景 Persist 有同名碰撞坑,v1 不踩;真·分场景切换留 v2)。区域归属靠坐标(x<边界=家园,x≥=野外)。
